...
impl<'a> TodoRepository<'a> {
    ...
    pub fn create(&self, input: &CreateTodoInput) -> DatabaseResult<Todo> {
        ...
        let id = self.db.execute_in_transaction(|tx| {
            tx.execute(
                r#"
                INSERT INTO todos (title, description, due_date, completed, priority, calendar_id, created_at, updated_at)
    ...
    pub fn get_all(&self) -> DatabaseResult<Vec<Todo>> {
        self.db.execute(|conn| {
            let mut stmt = conn.prepare(
                r#"
                SELECT id, title, description, due_date, completed, priority, calendar_id, created_at, updated_at
                FROM todos
    ...
    pub fn get_by_id(&self, id: i64) -> DatabaseResult<Option<Todo>> {
        self.db.execute(|conn| {
            ...
            let result = stmt.query_row(params![id], Self::from_row);
            match result {
    ...
    pub fn get_by_calendar_id(&self, calendar_id: i64) -> DatabaseResult<Vec<Todo>> {
        self.db.execute(|conn| {
            ...
            let todos = stmt
                .query_map(params![calendar_id], Self::from_row)?
                .filter_map(|r| r.ok())
                .collect();
            Ok(todos)
        })
