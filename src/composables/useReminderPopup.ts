...
import { availableMonitors, PhysicalPosition, type Monitor } from '@tauri-apps/api/window'
...
/**
 * 获取精简面板当前位置
 * @returns 精简面板位置，如果未显示则返回 null
 */
async function getCalendarPopupPosition(): Promise<{ x: number; y: number } | null> {
  try {
...
/**
 * 根据精简面板位置调整提醒窗口位置
 * 如果精简面板显示在右下角，提醒窗口上移以避免重叠
 * @param basePosition 基础位置（右下角）
 * @param calendarPopupPosition 精简面板位置，如果未显示则为 null
 * @param taskbarPosition 任务栏位置，用于决定上移方向
 * @returns 调整后的位置
 */
export function adjustPositionForPopup(
  basePosition: { x: number; y: number },
  calendarPopupPosition: { x: number; y: number } | null,
  taskbarPosition: TaskbarPosition = 'bottom'
): { x: number; y: number } {
  if (!calendarPopupPosition) {
    // 精简面板未显示，使用基础位置
    return basePosition
  }
...
export function checkAndAdjustBounds(
  ...
  // 右边界检查
  if (x + REMINDER_WIDTH > workRight - POPUP_MARGIN) {
    x = workRight - REMINDER_WIDTH - POPUP_MARGIN
  }
...
export async function positionReminderWindow(reminderWindow: WebviewWindow): Promise<void> {
  ...
  // 检查精简面板位置
  const calendarPopupPosition = await getCalendarPopupPosition()

  // 根据精简面板和任务栏位置调整
  const adjustedPosition = adjustPositionForPopup(basePosition, calendarPopupPosition, taskbar.position)

  // 边界检查（使用工作区域）
  const finalPosition = checkAndAdjustBounds(adjustedPosition, primaryMonitor)
  ...
  if (calendarPopupPosition) {
    console.log(`[useReminderPopup] 精简面板显示在 (${calendarPopupPosition.x}, ${calendarPopupPosition.y})，提醒窗口${taskbar.position === 'top' ? '下移' : '上移'} ${POPUP_OFFSET}px`)
  }
}
