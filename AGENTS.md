...
#### 6.5 开发完成后的评论检查
开发完成并创建 PR 后，必须执行以下检查流程：
1. **重新获取 PR 和 Issue**: 使用 `gh pr view` 和 `gh issue view` 命令获取最新状态
2. **检查非智能体评论**: 查看是否有用户或其他开发者的评论、反馈或修改要求
3. **响应评论**: 根据评论内容进行必要的代码修改或回复说明
4. **循环检查**: 修改完成后再次检查，直到没有新的非智能体评论需要处理
5. **确认完成**: 当 PR 和 Issue 无待处理评论时，视为功能开发结束
